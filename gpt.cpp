...
float* run(char *text, gpt2_model & model, gpt_vocab & vocab) {
    ...
    gpt_params params;
    params.model = "/Volumes/Documentos/Models/gpt2-small-portuguese/ggml-model-f32.bin";
    ...
    for (int i = embd.size(); i < embd_inp.size() + params.n_predict; i++) {
        ...
        if (embd.size() > 0) {
            ...
            embeddings = gpt2_eval(model, params.n_threads, n_past, embd, logits, mem_per_token);
