...
std::vector<std::vector<gpt_vocab::id>> split_sentences(const std::vector<gpt_vocab::id> &tokens, const gpt_vocab &vocab)
{
    ...
    for (auto token_iter = tokens.begin(); token_iter != tokens.end(); ++token_iter)
    {
        ...
        if (token == period_token || token == question_token || token == exclamation_token)
        {
            auto next_token_iter = std::next(token_iter);
            if (next_token_iter != tokens.end())
            {
                size_t next_sentence_length = 0;
                for (auto iter = next_token_iter; iter != tokens.end() && (*iter != period_token && *iter != question_token && *iter != exclamation_token); ++iter)
                {
                    ++next_sentence_length;
                }

                if (current_sentence.size() + next_sentence_length > max_tokens)
                {
